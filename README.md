...
Codex has no lifecycle hooks and does not natively load modular Skills, so AI-Brain loading relies on prompt-level instructions in `AGENTS.md` plus the linked Skill files for reference and reuse.
...
| OpenAI Codex | ✅ | ⚠️ AGENTS.md + linked Skills | ✅ | ⚠️ Prompt-driven |
...
Codex 沒有生命週期 hook，也不會原生載入模組化 Skills，所以 AI-Brain 載入主要依賴 `AGENTS.md` 內的提示詞規則，並搭配 symlink 進 `~/.codex/skills/` 的 Skill 檔作為參考與重用。
...
| OpenAI Codex | ✅ | ⚠️ AGENTS.md + symlink Skills | ✅ | ⚠️ 以提示詞驅動 |
